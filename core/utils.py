...
def draw_bbox(image, bboxes, disabled=0, show_label=True):
    if disabled == 0:
      ...
      classes=read_class_names(cfg.YOLO.CLASSES_1)
      num_classes = len(classes)
      image_h, image_w, _ = image.shape
      hsv_tuples = [(0.3, 1., 1.)]
    ...
    #print(out_boxes) [0.408841   0.02074599 0.78156394 0.32313377]
    for i in range(num_boxes[0]):
        ...
        if classes[class_ind] == 'license_plate':
            # electronic car detector
            elec_plate = image[int(coor[0]):int(coor[2]),int(coor[1]):int(coor[3])]
            #cv2.imshow("lp",elec_plate)
            #cv2.waitKey(0)
            
            # image[ymin: ymin + h, xmin: xmin + w]
            isElectronic = 0 
            notElectronic = 0
            ...
            if 3 * isElectronic >= notElectronic: # 10배(하이퍼 파라미터) 이상 차이가 나면 전기자동차로 인식
                ...
                cv2.rectangle(image, c1, c2, (255, 255, 255), bbox_thick)
                cv2.rectangle(image, c1, (int(np.float32(c3[0])), int(np.float32(c3[1]))), (255, 255, 255), -1)
